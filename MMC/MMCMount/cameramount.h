...
public:
    ...
    typedef enum CAM_DNDATA_TYPE
    {
        ...
        CAM_DN_COLOR_PLATE=0x20,//色板轮询 [主要是红外]
    } CAM_DNDATA_TYPE;
    ...
    Q_PROPERTY(int     zoomPosition READ zoomPosition   NOTIFY zoomPositionChanged) //变焦距离
    ...
    Q_PROPERTY(bool         debugOnline     READ debugOnline        WRITE setDebugOnline    NOTIFY debugOnlineChanged)  //云台在线调参数使能
    ...
    int     zoomPosition    (void){return _zoomPosition;}
    /* 无以下功能时 -- 屏蔽即可 */
    ...
    int     debugOnline     (void){return _debugOnline;}
    ...
    /* 相机模式切换 */
    Q_INVOKABLE int  switchCameraMode(); //由于状态不返回 直接本地判断
    Q_INVOKABLE void switchColor();      //色板轮询 -- 目前支持 【红外 - 海视英科红外】

    virtual void    setCam_type     (int type);
    int     mountType(void) const final { return MOUNT_CAM_INFO; }
    void    handleInfo(const can_data& data);//云台类
    void    setDebugOnline(bool debugOnline);
...
signals:
    ...
    void debugOnlineChanged();
    void zoomStateChanged();

private slots:
    void    oninitVideoUseClick();
    void    onSwitchColorSend();  //发送色板切换
private:
    ...
    void    setZoomPosition (int value);
...
//    void    sendTime();
    ...
    /* 子类有该功能 需重写实现*/
    // 解析反馈温度信息
    virtual void parseTemCode(const uint8_t* ptr, int length) { Q_UNUSED(ptr); Q_UNUSED(length);}
